...
export default function NoterScreen() {
        ...
        if (parts) {
          ...
          for (const uid of autresIds) {
            const { data: u } = await supabase
              ...
              .eq('id', uid)
              .single();
            if (u) participants.push({ user_id: uid, prenom: u.prenom });
          }
  ...
  const setNote = (userId: string, note: number) => {
    setNotes((prev) => ({ ...prev, [userId]: note }));
  };

  const setCommentaire = (userId: string, texte: string) => {
    setCommentaires((prev) => ({ ...prev, [userId]: texte }));
  };
  ...
  const envoyerNotes = async () => {
    ...
    if (notesAEnvoyer.length === 0) {
      Alert.alert('Info', 'Note au moins un participant !');
      return;
    }
    ...
    try {
      for (const p of notesAEnvoyer) {
        await supabase.from('notations').insert({
          activite_id: id,
          noteur_id: userId,
          note_cible_id: p.user_id,
          note: notes[p.user_id],
          commentaire: commentaires[p.user_id] || null,
        });

        const { data: userCible } = await supabase
          .from('utilisateurs')
          .select('note_moyenne')
          .eq('id', p.user_id)
          .single();
      ...
      setEnvoyé(true);
      Alert.alert('🎉 Merci !', 'Tes notes ont été envoyées !', [
  ...
  const EtoilesNote = ({ userId, note }: { userId: string; note: number }) => (
    <View style={styles.etoilesRow}>
      {[1, 2, 3, 4, 5].map((star) => (
        <TouchableOpacity key={star} onPress={() => setNote(userId, star)}>
          <Text style={[styles.etoile, note >= star && styles.etoileActive]}>
            ★
          </Text>
        </TouchableOpacity>
      ))}
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.participantHeader}>
                ...
                <View style={styles.participantInfo}>
                  ...
                  <Text style={styles.participantSub}>
                    {notes[p.user_id] ? `${notes[p.user_id]}/5 ⭐` : 'Pas encore noté'}
                  </Text>
                </View>
              </View>
      ...
      {participants.length > 0 && (
        <TouchableOpacity
          style={[styles.bouton, envoyé && styles.boutonDisabled]}
          onPress={envoyerNotes}
          disabled={envoyé}>
          <Text style={styles.boutonTexte}>
            {envoyé ? '✅ Notes envoyées !' : '⭐ Envoyer mes notes'}
          </Text>
        </TouchableOpacity>
      )}
