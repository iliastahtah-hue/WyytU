...
const CATEGORIES = [
  { label: 'Tout', emoji: '✨', gradient: ['#667EEA', '#764BA2'] },
  { label: 'Sport', emoji: '⚡', gradient: ['#FF416C', '#FF4B2B'] },
  { label: 'Resto', emoji: '🍕', gradient: ['#F7971E', '#FFD200'] },
  { label: 'Ciné', emoji: '🎬', gradient: ['#8E2DE2', '#4A00E0'] },
  { label: 'Soirée', emoji: '🎉', gradient: ['#FC466B', '#3F5EFB'] },
  { label: 'Gaming', emoji: '🎮', gradient: ['#0072FF', '#00C6FF'] },
  { label: 'Voyage', emoji: '✈️', gradient: ['#00B4DB', '#0083B0'] },
  { label: 'Musique', emoji: '🎵', gradient: ['#11998E', '#38EF7D'] },
  { label: 'Bien-être', emoji: '🏃', gradient: ['#56AB2F', '#A8E063'] },
  { label: 'Social', emoji: '👥', gradient: ['#FF416C', '#FF4B2B'] },
  { label: 'Art', emoji: '🎨', gradient: ['#F7971E', '#FFD200'] },
];
...
const getGradient = (cat: string) => CAT_GRADIENT[cat] || ['#667EEA', '#764BA2'];
...
const getMissions = (ville: string) => {
  ...
  return [
    { emoji: '☕', gradient: ['#FF6B35', '#F7971E'], temps: 'Dans 30 min', titre: `Café à ${v}`, desc: 'Bonne conversation', participants: 2 },
    { emoji: '⚽', gradient: ['#FF416C', '#FF4B2B'], temps: 'Dans 1h', titre: `Foot 5v5 — ${v}`, desc: 'Il manque 2 joueurs', participants: 5 },
    { emoji: '🏃', gradient: ['#11998E', '#38EF7D'], temps: 'Dans 45 min', titre: 'Running 5km', desc: 'Tous niveaux bienvenus', participants: 3 },
    { emoji: h >= 18 ? '🎉' : '🎬', gradient: h >= 18 ? ['#FC466B', '#3F5EFB'] : ['#8E2DE2', '#4A00E0'], temps: h >= 18 ? 'Ce soir' : 'Cet après-midi', titre: h >= 18 ? `Soirée à ${v}` : `Ciné — ${v}`, desc: h >= 18 ? 'Ambiance garantie 🔥' : 'Film au choix', participants: 4 },
  ];
};
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <TouchableOpacity key={cat.label} onPress={() => setCategorieActive(cat.label)} activeOpacity={0.8}>
                {active ? (
                  <LinearGradient colors={cat.gradient as [string,string]} style={s.catActive} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.missionsScroll}>
            {missions.map((m, i) => (
              <TouchableOpacity key={i} style={s.missionCard} onPress={() => router.push('/creer-activite' as any)} activeOpacity={0.88}>
                <LinearGradient colors={m.gradient as [string,string]} style={s.missionGrad}>
                ...
                {activitesFiltrees.slice(0, 5).map(a => {
                  ...
                  return (
                    <TouchableOpacity key={a.id} style={s.featuredCard} onPress={() => router.push(`/activite/${a.id}` as any)} activeOpacity={0.88}>
                      <LinearGradient colors={gradient as [string,string]} style={s.featuredInner}>
...
const s = StyleSheet.create({
  ...
  modalHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: C.border, alignSelf: 'center', marginTop: 12, marginBottom: 0 },
