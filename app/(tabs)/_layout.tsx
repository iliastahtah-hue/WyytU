...
const TAB_ICONS = {
  ...
  carte: { active: '🗺️', inactive: '🗺️', label: 'Carte' },
...
function TabIcon({ name, focused }: { name: keyof typeof TAB_ICONS; focused: boolean }) {
  ...
  return (
    <View style={s.itemWrapper}>
      {focused ? (
        <LinearGradient colors={[C.goldLight, C.gold]} style={s.iconBubbleActive} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      ...
      <Text style={[s.label, { color: focused ? C.gold : C.inactive }]}>{tab.label}</Text>
    </View>
  );
}
...
export default function TabLayout() {
  return (
    <Tabs screenOptions={{ headerShown: false, tabBarStyle: s.tabBar, tabBarItemStyle: s.tabItem }}>
      ...
      <Tabs.Screen name="groupe" options={{ href: null }} />
    </Tabs>
  );
}
...
const s = StyleSheet.create({
  ...
  itemWrapper: { alignItems: 'center', gap: 5 },
  iconBubble: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.06)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
